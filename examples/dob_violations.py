...
def run():
    ...
    unit, groups, (G, B, T) = convoys.utils.get_arrays(
        df, groups="type", created="issue_date", converted="disposition_date", unit="years", group_min_size=100
    )
    ...
    unit, groups, (G, B, T) = convoys.utils.get_arrays(
        df, groups="bucket", created="issue_date", converted="disposition_date", unit="years", group_min_size=500
    )
    convoys.plotting.plot_cohorts(G, B, T, model="kaplan-meier", groups=groups, t_max=30, ci=0.95)
    convoys.plotting.plot_cohorts(G, B, T, model="weibull", groups=groups, t_max=30, plot_kwargs={"linestyle": "--"})
    pyplot.legend()
    pyplot.xlabel(unit)
