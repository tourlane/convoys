...
def generalized_gamma_loss(x, X, B, T, W, fix_k, fix_p, hierarchical, flavor, callback=None):
...
class GeneralizedGamma(RegressionModel):
    ...
    **Solving the optimization problem**
    ...
    def __init__(self, ci=False, fix_k=None, fix_p=None, hierarchical=True, flavor="logistic"):
        self._ci = ci
        self._fix_k = fix_k
        self._fix_p = fix_p
    ...
    def rvs(self, x, n_curves=1, n_samples=1, T=None):
        ...
        for i, j in enumerate(numpy.random.randint(len(params["k"]), size=n_curves)):
            ...
            cdf_now = c * gammainc(k, numpy.multiply.outer(T[i], lambd) ** p)  # why is this outer?
