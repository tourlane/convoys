...
class KaplanMeier(SingleModel):
    ...
    def fit(self, B, T):
        ...
        BT = [(b, t) for b, t in zip(B, T) if t >= 0 and 0 <= float(b) <= 1]
        if len(BT) < len(B):
            n_removed = len(B) - len(BT)
            warnings.warn(
                "Warning! Removed %d/%d entries from inputs where " "T < 0 or B not 0/1" % (n_removed, len(B))
            )
    ...
    def _get_value_at(self, j, ci):
        if ci:
            ...
            return (
                1 - self._ss[j],
                1 - numpy.exp(-numpy.exp(numpy.log(-numpy.log(self._ss_clipped[j])) + z_hi * self._vs[j] ** 0.5)),
                1 - numpy.exp(-numpy.exp(numpy.log(-numpy.log(self._ss_clipped[j])) + z_lo * self._vs[j] ** 0.5)),
            )
        else:
            return 1 - self._ss[j]
